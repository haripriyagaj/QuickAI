...
const BlogTitles = () => {
  ...
  const blogCategories = [ 'General', 'Technology', 'Health', 'Lifestyle', 'Education', 'Travel', 'Food', 'Business'
    ]
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 
        text-slate-700'>
          {/* left col */}
          <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg borde border-gray-200'>
            ...
            <input onChange={(e)=>setInput(e.target.value)} value={input} type='text' className='w-full p-2 mt-2 outline-none text-sm border border-gray-300 rounded-md' placeholder='Enter article topic' required />
